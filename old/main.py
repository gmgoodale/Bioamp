    ...
    def updateChannelsContinuosly(self):
        ...
        with nidaqmx.Task() as readTask:
            ...
            readTask.timing.cfg_samp_clk_timing(self.samplingRate,
                sample_mode=AcquisitionType.CONTINUOUS, samps_per_chan=self.samplingRate)
            readTask.start()
            ...
            holder_array = numpy.zeros((self.numberOfChannles, 4),
                dtype=numpy.float64)
            ...
            while getattr(NIDAQThread, "continueRunning", True):
                ...
                # Waits until 2 samples are availabe and then reads them. Althrough
                # it seems nicer to read all available samples, nidaq currently has buffer sizing issues
                samplesRead = self.reader.read_many_sample(holder_array, number_of_samples_per_channel=4)

                #self.file.SaveData(self.timeElapsed, holder_array)

                for i in range(self.numberOfChannles):
                        self.channelBuffers[i].append((self.timeElapsed, holder_array[i][0]))
                        self.channelBuffers[i].append((self.timeElapsed + (1/self.samplingRate), holder_array[i][1]))
                        self.channelBuffers[i].append((self.timeElapsed + 2*(1/self.samplingRate), holder_array[i][2]))
                        self.channelBuffers[i].append((self.timeElapsed + 2*(1/self.samplingRate), holder_array[i][3]))
                        #print("Channel " + str(i) + ": " + str(self.channelBuffers[i]), flush=True)

                self.timeElapsed += 4*(1/self.samplingRate)
...
class GraphValues(Widget):
    ...
    def updateGraph(self, dt):
        ...
        tempBuffers = self.DAQ.channelBuffers
        ...
        for timePlot, timeVals in zip(self.timePlots, tempBuffers):
